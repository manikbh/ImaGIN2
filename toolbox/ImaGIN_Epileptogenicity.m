...
try
    ...
    AnalysisType = spm_input('Type of output', '+1', 'Seizure|Semiology');
end
...
switch AnalysisType
    ...
    case 'Semiology'
        NameEpileptogenicity='SI';
        
            % ===== PROCESS EACH FILE SEPARATELY =====
            for i0 = 1:size(DD,1)
                Latency=latency(i0,:);
                Duration=Horizon(i0,:);
                if sum(Latency)>0   %The symptom is present in the semiology of the seizure
                    TimeWindow = 0 : TimeResolution : max(Latency+Duration)+1;
                    ...
                    % Save Log Power: Seizure
                    D1 = clone(D, [FileName spm_str_manip(D.fname,'s') '_' NameEpileptogenicity '_' num2str(min(FreqBand)) '_' num2str(max(FreqBand)) '.mat'],[D.nchannels size(Epileptogenicity,2) 1]);
                    D1(:,:,:) = Epileptogenicity;
                    D1 = fsample(D1,1/(DPower.tf.time(2)-DPower.tf.time(1)));
                    D1 = timeonset(D1,min(DPower.tf.time));
                    save(D1);
                    % Save Log Power: Baseline
                    D1 = clone(B, [FileName spm_str_manip(D.fname,'s') '_' NameEpileptogenicity 'Baseline_' num2str(min(FreqBand)) '_' num2str(max(FreqBand)) '.mat'],[B.nchannels size(EpileptogenicityBaseline,2) 1]);
                    D1(:,:,:) = EpileptogenicityBaseline;
                    D1 = fsample(D1,1/(DPowerBaseline.tf.time(2)-DPowerBaseline.tf.time(1)));
                    D1 = timeonset(D1,min(DPowerBaseline.tf.time));
                    save(D1);
                    ...
                    clear SS
                    SS.TimeWindow=[];
                    for i1=1:length(Latency)
                        SS.TimeWindow = [SS.TimeWindow Latency(i1) + (0:TimeResolution:Duration(i1))];
                    end
                    SS.MeshFile = MeshFile;
                    SS.TimeWindowWidth = 0;
                    SS.SizeHorizon = 10;
                    dirSeizure  = fullfile(P,[FileName spm_str_manip(D.fname,'s') '_' NameEpileptogenicity '_' num2str(min(FreqBand)) '_' num2str(max(FreqBand))]);
                    dirBaseline = fullfile(P,[FileName spm_str_manip(D.fname,'s') '_' NameEpileptogenicity 'Baseline_' num2str(min(FreqBand)) '_' num2str(max(FreqBand))]);
                    % Save volume (.nii) or surface (.gii)
                    switch lower(OutputType)
                        case 'volume'
                            % Define volume options
                    ...
                    % SPMs
                    spmDir = fullfile(P,['SPM_' NameEpileptogenicity '_' FileName '_' spm_str_manip(D.fname,'s') '_' num2str(min(FreqBand)) '_' num2str(max(FreqBand))]);
                    if isdir(spmDir)
                        cd(P)
                        rmdir(spmDir,'s')
                    end
                    ...
                    WriteTvalues(deblank(DD(i0,:)), ...  % Reference recordings
                        fullfile(matlabbatch{1}.spm.stats.fmri_spec.dir{1}, ['spmT_0001' outExt]), ...  % T-values
                        fullfile(P, [NameEpileptogenicity '_' spm_str_manip(D.fname,'s') '_' FileName '_' num2str(min(FreqBand)) '_' num2str(max(FreqBand))]), ... % Output file name (without the extension)
                        OutputType, giiCortex, SaveMNI, VolRes);   % 'volume' or 'surface'
                end
            end
            ...
            % If there are multiple files in input
            if (size(DD,1) > 1)
                if find(sum(latency,2))>1
                    % SPM of GI
                    spmDir = fullfile(P,['SPM_' NameEpileptogenicity '_Group_' FileName '_' num2str(min(FreqBand)) '_' num2str(max(FreqBand))]);
                    if isdir(spmDir)
                        cd(P);
                        rmdir(spmDir,'s');
                    end
                    ...
                    WriteTvalues(deblank(DD(1,:)), ...  % Reference recordings
                        fullfile(matlabbatch{1}.spm.stats.fmri_spec.dir{1}, ['spmT_0001' outExt]), ...  % T-values
                        fullfile(P, [NameEpileptogenicity '_Group_' FileName '_' num2str(min(FreqBand)) '_' num2str(max(FreqBand))]), ... % Output file name (without the extension)
                        OutputType, giiCortex, SaveMNI, VolRes);   % 'volume' or 'surface'
                end
            end
            ...
            for i0 = 1:size(DD,1)
                D = spm_eeg_load(deblank(DD(i0,:)));
                P = spm_str_manip(deblank(DD(i0,:)),'h');
                rmdir(fullfile(P,[FileName spm_str_manip(D.fname,'s') '_' NameEpileptogenicity '_' num2str(min(FreqBand)) '_' num2str(max(FreqBand))]),'s')
                rmdir(fullfile(P,[FileName spm_str_manip(D.fname,'s') '_' NameEpileptogenicity 'Baseline_' num2str(min(FreqBand)) '_' num2str(max(FreqBand))]),'s')
            end
